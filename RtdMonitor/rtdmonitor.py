...
class RtdCommonPlotter(RtdPlotterBase):
    ...
    def update(self):
        ...
        for n, ax_name in enumerate(l_ax_names):
            ...
            # 清空原有的 子图数据
            self._ax_list[n].clear()

            # 按column 画线
            for _column_name in list(df.columns):
                _l_values = list(df.loc[:, _column_name])
                # ax.scatter(_l_tickers, _l_value, label=_trader, s=2)
                self._ax_list[n].plot(
                    l_index_sorted_by_value, _l_values,
                    label=_column_name, linewidth=0.5,
                )

            # 横纵轴 标签
            # ax.set_xticks(all_tickers)
            self._ax_list[n].set_xticklabels(l_index_sorted_by_value, rotation=90, fontsize=7)
            self._ax_list[n].set_ylim((min_value, max_value),)
            self._ax_list[n].yaxis.label.set_size(7)
            self._ax_list[n].set_title(
                f"{ax_name}_{str(len(l_column_names))}_{datetime.now().strftime('%H:%M:%S')}",
                fontsize=8
            )
            # 图例
            plt.legend(bbox_to_anchor=(1., 1.0), loc='upper left', fontsize=7)
            # plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left', fontsize=7)
            # plt.legend(bbox_to_anchor=(0, 1.0), fontsize=7)
